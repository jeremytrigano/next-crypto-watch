    ...
    <Layout page="Crypto Watch - Accueil">
      <ul className="flex justify-around py-10">
        {res.map((crypto, index) => (
          <li
            key={index}
            className="relative hover:shadow-md p-8 border border-purple-300 rounded-3xl bg-purple-100 md:w-auto flex-1 mx-5"
          >
            <Link href={`/${crypto.id}`}>
...
export async function getStaticProps(context) {
  try {
    const res = await fetch(
      `http://api.nomics.com/v1/currencies/ticker?key=${process.env.NOMICS_API_KEY}&ids=BTC,ETH,LTC&interval=1d,30d,365d`
    ).then((res) => res.json());
